...
def calculate_eff(cut_BDT: float=0.919, cut_3CNN: float=0.0, Luminosity: int=3000, b_tag_eff: float=0.77**4) -> pd.DataFrame:
    ...
    process_eff = {
                    ...
                    "length_MC" : np.zeros(len(process)),
                }
    ...
    for j , element in enumerate(process):
        ...
        Preselection
        """
        process_eff["preselection"][j] = len(tmp)/process_mclength[element]#*expected_event[element]

        """
        1100 GeV > M(J1,J2) > 900 GeV
        """
        tmp = tmp[(tmp["MJJ"] > 900) & (tmp["MJJ"] < 1100)]
        process_eff["MJJ"][j] = len(tmp)/process_mclength[element]#*expected_event[element]

        """
        4b-tag 
        """
        tmp = tmp[(tmp["four_b_tag"] == 1)]
        process_eff["four_b_tag"][j] = len(tmp)/process_mclength[element]#*expected_event[element]
        ...
        process_eff["Delta_Eta"][j] = len(tmp)/process_mclength[element]#*expected_event[element]
        ...
        """
        # tmp = tmp[(tmp["Xhh"] < 1.6)]   
        process_eff["XHH"][j] = len(tmp)/process_mclength[element]#*expected_event[element]
        ...
        process_eff["BDT"][j] = len(tmp)/process_mclength[element]#*expected_event[element]
        ...
        process_eff["3CNN"][j] = len(tmp)/process_mclength[element]#*expected_event[element]


        process_eff["survival"][j] = expected_event[element]*process_eff["3CNN"][j]
        
        process_eff["length_MC"][j] = len(tmp)
    ...
    preselection_process = pd.DataFrame(preselection_process,
                ...
                # index=["preselection","BDT","3CNN","MJJ","four_b_tag","survival","length_MC"]
                index=["preselection","MJJ","four_b_tag","Delta_Eta","XHH","BDT","3CNN","survival","length_MC"]
                )
...
# # %%
# selection_2 = calculate_eff(cut_BDT=0.94, cut_3CNN=0.0, Luminosity=3000, b_tag_eff=0.77**4)
# selection_2
# %%
